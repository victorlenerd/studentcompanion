...
import { GetCoursesOffline, GetCourses, SetCourses, SetCurrentCourse , GetCoursesByOtherId, SaveCourseOffline} from '../ducks/Courses';
...
class SavedCourses extends Component {
    ...
    _setAcademicInfo(universityId, facultyId, departmentId, levelId, userId) {
        ...
        this.props.setAcademicInfo(userId, {
            universityId,
            facultyId,
            departmentId,
            levelId
                ...
                let saveCoursesOffline = courses.map((course, index)=> {
                    return new Promise((resolve, reject)=> {
                        ...
                        .then((results)=> {
                            ...
                            this.props.saveCourseOffline(course)
                            .then(()=> {
                                Promise.all([
                ...
                <View style={{flexDirection: "column", flex: 1}}>
                    <ScrollView style={{flex:1}}>
                        {this.props.courses.map((course, index)=> {
                            if (course) {
                                return (
                                    <TouchableOpacity key={index} style={{padding: 20, marginBottom: 2, backgroundColor: colors.white}} onPress={()=> { this._openCourse(course);}}>
