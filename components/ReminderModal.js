...
class ReminderModal extends Component {
    ...
    componentDidMount(event) {
        ...
        let zeroPrefix = (num)=>  { return (num < 10) ? `0${num}` : `${num}`; } 
    ...
    render () {
        return (
            <View style={{
                ...
                borderTopColor: colors.accent, borderTopWidth: 2
            }}>

                <ScrollView style={{flex: 0.8, flexDirection: "column"}}>
                    <Text style={{fontSize: 18, textAlign: "center", fontWeight: "bold", marginBottom: 50, marginTop: 50}}>Choose Time</Text>
                    ...
                    <Text style={{fontSize: 18, textAlign: "center", fontWeight: "bold", marginBottom: 50, marginTop: 50}}>Repeat</Text>
                    {this.state.repeats.map((repeatType, i)=> {
                        return (
                            <TouchableOpacity key={i} onPress={()=> { this.setState({repeatType}); }} style={{flex: 1, paddingLeft: 20, flexDirection: "row", alignItems:"center", width, height: 60, borderTopColor: colors.gray, borderTopWidth: 1}}>
                                <View style={{
