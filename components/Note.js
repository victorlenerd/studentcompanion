...
let supportsQuill =()=> {
    ...
    return (deviceType == 'iOS' && parseFloat(deviceVersion) >= 9.0 || deviceType == 'Android' && parseFloat(deviceVersion) >= 5.1) ? true : false;
}
...
class Note extends Component {
    ...
    prev() {
        if (this.state.currentSentence >= 1 && this.state.currentSentence < this.state.sentences.length-1 && this.state.playing) {
            Tts.stop();
            let currentSentence =  this.state.currentSentence - 1;
            let startIndex = this.props.currentNote.text.indexOf(this.state.sentences[currentSentence]);
            let endIndex =  this.props.currentNote.text.indexOf(this.state.sentences[currentSentence + 1])
    ...
    renderCommentsCount() {
        ...
        if (this.state.comments.length > 0) {
            return (
                <View style={{width: 25, justifyContent: "center", alignItems: "center", height: 25, borderRadius: 12.5, backgroundColor: "#e74c3c", zIndex: 100, position: "absolute", top: -5, right: -10}}>
    ...
    render () {
        return (
            <View style={[main.container, {backgroundColor: (this.state.light) ? colors.white: colors.black, borderTopColor: colors.accent, borderTopWidth: 2 }]}>
                <View style={{width: width, backgroundColor: colors.black, paddingRight: 20, paddingLeft: 20, paddingTop: 20, flexDirection: "column", justifyContent:"space-between", alignItems: "flex-start", height: 80}}>
                    <Text style={{fontSize: 16, color: colors.white}}>Voice Rate</Text>
                    <Slider ref={(r)=> this.slider = r} style={{width:width-40}} value={0.5} onValueChange={this.rateChange.bind(this)} />
                </View>
                {this._renderReaderView()}
                <View style={{width, height: 60, flexDirection: "row", paddingLeft: 20, paddingRight: 20, justifyContent:"space-between", alignItems:"center"}}>
                    <TouchableOpacity 
                        style={{
                            width: 50,
                            height: 50,
                    ...
                    <TouchableOpacity 
                        ...
                        <View>
                            ...
                            <Image source={require('../assets/bubble-white.png')} style={{width: 22, height: 32}} resizeMode="contain" />
                        </View>
                    </TouchableOpacity>
                </View>
            </View>
